<template>
  <div>
    <a-space class="toolbar">
      <a-input-search v-model:value="keyword" placeholder="搜索订单号/供应商" style="width: 240px" @search="onSearch" />
      <a-button type="primary" v-permission="'purchase:add'" @click="openCreate">新增</a-button>
    </a-space>
      ...
      <a-table-column title="操作" width="240">
        <template #default="{ record }">
          <a-popconfirm v-if="record.status === '待审核'" title="确认审核通过?" @confirm="audit(record)">
            <a-button type="link" size="small" v-permission="'purchase:audit'">审核</a-button>
          </a-popconfirm>
          <a-popconfirm v-if="record.status === '已审核'" title="确认收货入库?库存将增加" @confirm="receive(record)">
            <a-button type="link" size="small" v-permission="'purchase:audit'">收货</a-button>
          </a-popconfirm>
          <a-button type="link" size="small" v-permission="'purchase:edit'" @click="openEdit(record)">编辑</a-button>
          <a-popconfirm title="确认删除?" @confirm="onDelete(record.id)">
            <a-button type="link" size="small" danger v-permission="'purchase:delete'">删除</a-button>
          </a-popconfirm>
        </template>
      </a-table-column>
    ...
    <a-drawer v-model:open="drawerOpen" :title="editing ? '编辑采购订单' : '新增采购订单'" :width="640">
      <a-form :model="form" layout="vertical">
        <a-form-item label="订单号" required><a-input v-model:value="form.orderNo" /></a-form-item>
        <a-form-item label="供应商" required><a-input v-model:value="form.supplierName" /></a-form-item>
...
const { list, loading, total, query, fetchList, create, update, remove } = useCrud<Purchase>('/scm/purchase')

const keyword = ref('')
const drawerOpen = ref(false)
const editing = ref<Purchase | null>(null)
const form = reactive<Partial<Purchase>>({ items: [] })
...
function statusColor(s: string) {
  return ({ '待审核': 'orange', '已审核': 'blue', '已入库': 'green' } as Record<string, string>)[s] ?? 'default'
}
...
function openCreate() {
  editing.value = null
  Object.assign(form, { orderNo: '', supplierName: '', warehouseId: undefined, warehouseName: '', date: '', status: '待审核', items: [] })
  drawerOpen.value = true
}
function openEdit(record: Purchase) {
  editing.value = record
  Object.assign(form, { ...record, items: (record.items || []).map((it) => ({ ...it })) })
  drawerOpen.value = true
}
async function onSubmit() {
  if (!form.orderNo || !form.supplierName || !form.warehouseId) {
    message.warning('请填写订单号、供应商、仓库')
    return
  }
...
async function audit(record: Purchase) {
  await update(record.id, { status: '已审核' })
  message.success('已审核')
}
async function receive(record: Purchase) {
  await request.post(`/scm/purchase/${record.id}/receive`, {
    warehouseId: record.warehouseId,
    warehouseName: record.warehouseName,
    items: record.items || [],
  })
  await update(record.id, { status: '已入库' })
  message.success('收货成功,库存已增加')
}
